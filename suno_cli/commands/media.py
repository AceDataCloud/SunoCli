...
@click.option("--json", "output_json", is_flag=True, help="Output raw JSON.")
@click.pass_context
def extract_vocals(ctx: click.Context, audio_id: str, vocal_start: float | None, vocal_end: float | None, callback_url: str | None, output_json: bool) -> None:
    ...
    client = get_client(ctx.obj.get("token"))
    try:
        result = client.get_vox(audio_id=audio_id, vocal_start=vocal_start, vocal_end=vocal_end, callback_url=callback_url)
        if output_json:
            print_json(result)
        else:
            url = result.get("data", {}).get("audio_url", "")
