...
@click.option("--json", "output_json", is_flag=True, help="Output raw JSON.")
@click.pass_context
def wav(ctx: click.Context, audio_id: str, output_json: bool) -> None:
...
@click.option("--json", "output_json", is_flag=True, help="Output raw JSON.")
@click.pass_context
def midi(ctx: click.Context, audio_id: str, output_json: bool) -> None:
    ...
    client = get_client(ctx.obj.get("token"))
    try:
        result = client.get_midi(audio_id=audio_id)
        if output_json:
            print_json(result)
        else:
...
@click.option("--json", "output_json", is_flag=True, help="Output raw JSON.")
@click.pass_context
def extract_vocals(ctx: click.Context, audio_id: str, output_json: bool) -> None:
    ...
    client = get_client(ctx.obj.get("token"))
    try:
        result = client.get_vox(audio_id=audio_id)
        if output_json:
            print_json(result)
        else:
            url = result.get("data", {}).get("audio_url", "")
